...
Running `azd up` without setting `RECIPE` launches an interactive setup wizard (`hooks/preprovision.sh` on macOS/Linux, `hooks/preprovision.ps1` on Windows) that walks through all configuration with arrow-key selectors. Current values are pre-selected — press Enter to keep them.
...
| [Git](https://git-scm.com)                                                                           | pre-installed            | `winget install Git.Git`            | pre-installed                                             |
| `kubectl`                                                                                                | `brew install kubectl`   | `winget install Kubernetes.kubectl` | `az aks install-cli`                                      |

**Additional tools** (needed by macOS/Linux deploy scripts, validated automatically):

- `envsubst` - macOS: `brew install gettext` · Linux: `apt-get install gettext-base` · Windows: **not needed** (PowerShell hooks implement template substitution natively)
- `openssl` - macOS: `brew install openssl` · Linux: `apt-get install openssl` · Windows: **not needed** (PowerShell hooks use .NET crypto for TLS certificate generation)
...
| **Windows (PowerShell / cmd)** | `azd` runs native `.ps1` hooks — no bash/Git Bash needed                                      |
| **Windows (Git Bash)**         | `azd` runs `.sh` hooks directly (same as Mac/Linux)                                            |
| **WSL**                        | Same as Linux                                                                                  |

The `.ps1` hooks are full native PowerShell implementations (not wrappers around bash). They implement the same wizard, deployment, and postprovision logic as the `.sh` scripts — including `envsubst`-style template substitution (via `Invoke-EnvSubst`) and TLS certificate generation (via .NET crypto, no `openssl` required).
...
# Or run directly
pwsh hooks/deploy.ps1
pwsh hooks/deploy.ps1 -y    # skip confirmation
```
...
Each hook validates its prerequisites before running. Both `.sh` (macOS/Linux) and `.ps1` (Windows) versions perform the same checks:

| Script                          | CLI tools                              | Az extensions                                                     | Env vars                                             | Cluster context   |
| ------------------------------- | -------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------- | ----------------- |
| `hooks/preprovision`            | `az`, `azd`                            | -                                                                 | `ARC_PREFIX`, `NODE_COUNT`, `VM_SIZE`, `DEPLOY_MODE` | -                 |
| `hooks/postprovision`           | `az`, `azd`                            | -                                                                 | - (delegates)                                        | -                 |
| `hooks/deploy`                  | `az`, `azd`                            | -                                                                 | - (auto-loads)                                       | -                 |
| `k8s/postprovision`             | `az`, `kubectl`                        | `connectedk8s`                                                    | `AZURE_LOCATION`, `AZURE_WI_CLIENT_ID`               | - (fetches creds) |
| `k8s/deploy`                    | `az`, `kubectl`, `envsubst`¹, `openssl`¹ | -                                                                 | `ARC_PREFIX`                                         | ✅ auto-fetches   |
| `containerapp/postprovision`    | `az`, `kubectl`                        | `connectedk8s`, `k8s-extension`, `customlocation`, `containerapp` | `AZURE_LOCATION`, `AZURE_WI_CLIENT_ID`               | - (fetches creds) |
| `containerapp/deploy`           | `az`, `kubectl`, `envsubst`¹           | `containerapp`                                                    | `ARC_PREFIX`                                         | ✅ auto-fetches   |

¹ `envsubst` and `openssl` are only required by the `.sh` scripts. The `.ps1` versions implement template substitution and TLS cert generation natively.
...
./hooks/deploy.sh       # fast redeploy — current settings, no provision (macOS/Linux)
...
```powershell
azd up                       # full wizard — modify settings + provision + deploy
pwsh hooks/deploy.ps1        # fast redeploy — current settings, no provision
pwsh hooks/deploy.ps1 -y     # instant redeploy, skip confirmation
$env:AUTO_YES="true"; azd up # skip all prompts (CI/automation)
azd down --force --purge     # tear down everything
```
...
### Shared Defaults (`infra/defaults.sh` / `infra/defaults.ps1`)

All hooks source `infra/defaults.sh` (macOS/Linux) or `infra/defaults.ps1` (Windows), the **single source of truth** for environment variable defaults. The `apply_defaults()` / `Apply-Defaults` function:
...
`infra/defaults.sh` (or `infra/defaults.ps1` on Windows) auto-detects everything from Azure (AKS, ACR, identity, AI hub) and reads deployment config from RG tags. `AI_MODE` is derived from hub existence — no tag needed. The wizard sees `PROVISION_DONE=true` and shows the Deploy/Modify choice instead of the full setup flow.
...
The Docker image supports **runtime configuration** via environment variables. Any `VITE_*` environment variable injected into the container is automatically available to the frontend without rebuilding. This allows the same image to be deployed with different settings across environments.

### How It Works

1. The `docker-entrypoint.sh` script runs before nginx starts
2. It reads **all** `VITE_*` environment variables from the container
3. It writes them to `/config.js` as `window.__RUNTIME_CONFIG__`
4. The app reads config via `env()` from `src/runtime.ts` with fallback: runtime -> build-time -> default

### Configurable Variables

Any `VITE_*` environment variable is automatically picked up. Below are the supported variables:

**Core (used by `src/config/runtime.ts`):**

| Variable | Default | Description |
|----------|---------|-------------|
| `VITE_API_URL` | `/api` | Backend API base URL |
| `VITE_API_MODE` | *(empty)* | API mode (`agents` for Agentic RAG) |
| `VITE_AGENT_ID` | *(empty)* | Default agent ID |

**App branding & behavior (override `src/config/constants.ts`):**
...
| `VITE_CHAT_USE_ROUTES` | `true` | Use URL routes for conversations |
| `VITE_CHAT_SHOW_PROMPT_STARTERS` | `true` | Show prompt suggestions on welcome screen |
| `VITE_CHAT_SHOW_MESSAGE_ICON` | `true` | Show icon in chat messages |
| `VITE_CHAT_MESSAGE_ICON` | `/copilot-icon.svg` | Chat message icon path |
| `VITE_SIDEBAR_SHOW_ICON` | `false` | Show icon in sidebar toggle |
| `VITE_SIDEBAR_ICON` | *(empty)* | Sidebar icon path |
| `VITE_NEW_CHAT_SHOW_ICON` | `true` | Show icon in new chat button |
| `VITE_NEW_CHAT_ICON` | `/new-chat-icon.svg` | New chat button icon path |
| `VITE_LAYOUT_MAX_WIDTH` | `950px` | Main content max width |
| `VITE_LAYOUT_CARD_COLUMNS` | `2` | Card grid column count |
| `VITE_COPILOT_MODE` | `canvas` | Copilot display mode |
...
When deployed via Helm (e.g., Edge-RAG helm chart), inject env vars through the pod spec. Any `VITE_*` variable is automatically picked up:
...
For local development or custom builds, you can still use `.env` files:

```sh
# .env.local
VITE_API_URL=/api
VITE_API_MODE=agents
VITE_AGENT_ID=asst_xxxx
```

Build-time values are baked into the JS bundle. Runtime env vars (via Docker) take precedence when present.
...
_Last updated: 2026-03-11 | Last commit: ce78188_
