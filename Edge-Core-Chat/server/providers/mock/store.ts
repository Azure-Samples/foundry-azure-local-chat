...
const conversationItems = new Map<string, Message[]>();
 ...
 */
const generateAzureId = (prefix: string): string => {
...
const generateConversationId = (): string => generateAzureId("conv");
const generateMessageId = (): string => generateAzureId("msg");
const generateResponseId = (): string => generateAzureId("resp");
...
export const mockStore = {
  ...
  addMessage(conversationId: string, role: "user" | "assistant", text: string): Message {
    ...
    conversationItems.set(conversationId, [...existing, msg]);
    return msg;
  },

  getConversationItems(conversationId: string): Message[] {
