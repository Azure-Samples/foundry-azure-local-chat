...
wizard_infrastructure() {
    ...
    local POP_VM_V=("Standard_B2s" "Standard_D2s_v3" "Standard_D4s_v3" "Standard_D2s_v5" "Standard_D4s_v5" "Standard_B4ms")
    local POP_VM_D=(
        "Standard_B2s ${DIM}— 2 vCPU, 4 GB (burstable, cheapest)${NC}"
        "Standard_D2s_v3 ${DIM}— 2 vCPU, 8 GB${NC} ${GREEN}(Recommended)${NC}"
        "Standard_D4s_v3 ${DIM}— 4 vCPU, 16 GB (prod)${NC}"
        "Standard_D2s_v5 ${DIM}— 2 vCPU, 8 GB (latest gen)${NC}"
        "Standard_D4s_v5 ${DIM}— 4 vCPU, 16 GB (latest gen)${NC}"
